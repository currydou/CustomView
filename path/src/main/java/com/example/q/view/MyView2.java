...
public class MyView2 extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawCircle(40, 40, 30, paint);//圆形
        canvas.drawRect(10, 90, 70, 150, paint);//正方形
        canvas.drawRect(10, 170, 70, 200, paint);//长方形
        canvas.drawOval(new RectF(10, 220, 70, 250), paint);//椭圆

        Path path = new Path();//三角形
        path.moveTo(10, 330);//基点点
        path.lineTo(70, 330);//画线
        path.lineTo(40, 270);
        path.close();
        canvas.drawPath(path, paint);//连线
        ...
        path1.close();//把开始的点和最后的点连接在一起，构成一个封闭图形
            /*
             * 最重要的就是moveTo和close,如果是Style.FILL的话，不设置close,也没有区别，可是如果是STROKE模式，
             * 如果不设置close,图形不封闭。
             *
             * 当然，你也可以不设置close，再添加一条线，效果一样。
             */
        canvas.drawPath(path1, paint);//梯形
        ///////////////////////////////////////第二列
